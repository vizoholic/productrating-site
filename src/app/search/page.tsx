...
// Build direct platform URL using seller name + product title
function getDirectBuyUrl(seller: string, productName: string): string {
  ...
  // Unknown seller — search Amazon India as default
...
function StarRow({ score }: { score: number }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
      <div style={{ display:'flex', gap:2 }}>
        {[1,2,3,4,5].map(i => (
          <svg key={i} width="13" height="13" viewBox="0 0 24 24"
            fill={i <= full ? '#10B981' : (i === full+1 && half) ? 'none' : '#E5E7EB'}
            stroke={i <= full ? '#10B981' : (i === full+1 && half) ? '#10B981' : '#E5E7EB'}
            strokeWidth="1.5">
            {i === full+1 && half
              ? <><defs><linearGradient id={`hg${i}`}><stop offset="50%" stopColor="#10B981"/><stop offset="50%" stopColor="#E5E7EB"/></linearGradient></defs>
      ...
      <span style={{ fontSize:14, fontWeight:800, color:'#10B981' }}>{clamped.toFixed(1)}</span>
...
function AiCard({ p, idx }: { p:AiProduct; idx:number }) {
  const medals = ['🥇','🥈','🥉']
  const rankLabels = ['#1 Best Pick','#2','#3','#4','#5','#6']
  const platRating = Math.min(5, Math.max(1, p.platform_rating || Math.min(5, p.rating + 0.3)))
  const aiRating = Math.min(5, Math.max(1, p.rating))
  const buyUrl = getDirectBuyUrl(p.seller, p.name)
  ...
  return (
    <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:12, overflow:'hidden', boxShadow:'0 1px 3px rgba(0,0,0,0.06)', transition:'all .15s', display:'flex', flexDirection:'column' }}
      ...
      {/* Rank banner */}
      <div style={{ background: idx === 0 ? '#2563EB' : idx === 1 ? '#6B7280' : idx === 2 ? '#B45309' : '#F3F4F6', padding:'6px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <span style={{ fontSize:12, fontWeight:700, color: idx <= 2 ? '#fff' : '#6B7280', textTransform:'uppercase', letterSpacing:'.5px' }}>{rankLabels[idx]}</span>
      ...
      <div style={{ padding:20, flex:1 }}>
        {/* Badge */}
        {p.badge && (
          <span style={{ display:'inline-block', fontSize:11, fontWeight:600, color:'#10B981', background:'#ECFDF5', border:'1px solid #A7F3D0', borderRadius:100, padding:'2px 10px', marginBottom:10 }}>
            ● {p.badge}
          </span>
        )}
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ marginBottom:4 }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                ...
                {p.reviews && <span style={{ fontSize:11, color:'#9CA3AF' }}>{p.reviews} reviews</span>}
              </div>
              <StarRow score={aiRating} />
            </div>

            {/* Platform vs Our score comparison */}
            <div style={{ marginTop:10, display:'grid', gridTemplateColumns:'1fr auto 1fr', gap:6, alignItems:'center' }}>
              <div style={{ background:'#FEF2F2', border:'1px solid #FECACA', borderRadius:8, padding:'8px 10px', textAlign:'center' }}>
                <div style={{ fontSize:10, color:'#EF4444', fontWeight:600, marginBottom:3, textTransform:'uppercase' }}>Platform rating</div>
                <div style={{ fontSize:18, fontWeight:800, color:'#EF4444' }}>{platRating.toFixed(1)}<span style={{ fontSize:12, fontWeight:400, color:'#9CA3AF' }}>/5</span></div>
                <div style={{ fontSize:10, color:'#9CA3AF', marginTop:2 }}>includes fake reviews</div>
              </div>
              <div style={{ textAlign:'center', color:'#9CA3AF', fontSize:18 }}>→</div>
              <div style={{ background:'#ECFDF5', border:'1px solid #A7F3D0', borderRadius:8, padding:'8px 10px', textAlign:'center' }}>
                <div style={{ fontSize:10, color:'#10B981', fontWeight:600, marginBottom:3, textTransform:'uppercase' }}>PR Score</div>
                <div style={{ fontSize:18, fontWeight:800, color:'#10B981' }}>{aiRating.toFixed(1)}<span style={{ fontSize:12, fontWeight:400, color:'#9CA3AF' }}>/5</span></div>
                <div style={{ fontSize:10, color:'#9CA3AF', marginTop:2 }}>fake reviews removed</div>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Buy button — direct platform link */}
      <a href={buyUrl} target="_blank" rel="noopener noreferrer"
        style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'13px 20px', background:'#2563EB', color:'#fff', fontSize:14, fontWeight:600, textDecoration:'none', transition:'background .15s' }}
...
function SerpCard({ p }: { p:SerpProduct }) {
  // Build direct platform URL from seller name instead of using Google Shopping link
  const directUrl = getDirectBuyUrl(p.source, p.title)
  const sellerName = p.source || 'Store'
    ...
    <a href={directUrl} target="_blank" rel="noopener noreferrer"
      ...
      <div style={{ padding:14, flex:1 }}>
        <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
          {p.thumbnail && (
            <img src={p.thumbnail} alt="" style={{ width:56, height:56, objectFit:'contain', borderRadius:8, background:'#F9FAFB', flexShrink:0, border:'1px solid #F3F4F6' }}
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:4, flexWrap:'wrap' }}>
              <span style={{ fontSize:12, color:'#9CA3AF', fontWeight:500 }}>{sellerName}</span>
            ...
            {p.rating !== null && p.rating !== undefined && p.rating > 0 && (
              <div style={{ marginTop:6 }}>
                <StarRow score={Math.min(5, p.rating)} />
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{ padding:'10px 14px', background:'#EFF6FF', borderTop:'1px solid #BFDBFE', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <span style={{ fontSize:12, color:'#2563EB', fontWeight:500 }}>Search on {sellerName}</span>
          ...
          {aiProducts.length>0&&(
            <div style={{marginBottom:36}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,flexWrap:'wrap'}}>
                ...
                <span style={{fontSize:12,color:'#6B7280',background:'#F3F4F6',borderRadius:100,padding:'2px 10px',border:'1px solid #E5E7EB'}}>Ranked · All scores out of 5</span>
              </div>
          ...
          {serpProducts.length>0&&(
            <div style={{marginBottom:36}}>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,flexWrap:'wrap'}}>
                ...
                <span style={{fontSize:12,color:'#6B7280',background:'#F3F4F6',borderRadius:100,padding:'2px 10px',border:'1px solid #E5E7EB'}}>Click → goes to seller website</span>
              </div>
