  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
      <div style={{ flex:1, height:6, background:'#E5E7EB', borderRadius:3, overflow:'hidden' }}>
        <div style={{ width:`${(score/5)*100}%`, height:'100%', background:'#10B981', borderRadius:3, transition:'width .4s' }} />
      </div>
      <span style={{ fontSize:14, fontWeight:800, color:'#10B981', minWidth:28 }}>{score.toFixed(1)}</span>
    </div>
  )
}

// Seller name → search URL for buying
function getBuyUrl(seller: string, productName: string): string {
  ...
  // fallback — google shopping India
  return `https://www.google.com/search?q=${q}+buy+india&tbm=shop`
}
    ...
    <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:12, overflow:'hidden', boxShadow:'0 1px 3px rgba(0,0,0,0.06)', transition:'all .15s', display:'flex', flexDirection:'column' }}
      ...
      <div style={{ padding:20, flex:1 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
          {p.badge
            ? <span style={{ display:'inline-block', fontSize:11, fontWeight:600, color:'#10B981', background:'#ECFDF5', border:'1px solid #A7F3D0', borderRadius:100, padding:'2px 10px' }}>● {p.badge}</span>
            : <span />
          }
        ...
        {p.rating > 0 && (
          <div style={{ marginBottom:14 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6, alignItems:'center' }}>
              <span style={{ fontSize:12, fontWeight:600, color:'#374151' }}>AI Score</span>
              {p.reviews && <span style={{ fontSize:12, color:'#9CA3AF' }}>{p.reviews} reviews</span>}
            </div>
            <ScoreBar score={p.rating} />

            {/* Clearer explanation of rating adjustment */}
            <div style={{ marginTop:8, background:'#F9FAFB', borderRadius:8, padding:'8px 12px', border:'1px solid #F3F4F6' }}>
              <div style={{ fontSize:11, color:'#6B7280', marginBottom:4, fontWeight:500 }}>How we calculate this score:</div>
              <div style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, flexWrap:'wrap' }}>
                <span style={{ color:'#EF4444', fontWeight:500 }}>Raw: {(p.rating + 0.4).toFixed(1)} ⭐ on platform</span>
                <span style={{ color:'#9CA3AF' }}>→</span>
                <span style={{ color:'#10B981', fontWeight:700 }}>After removing fake reviews: {p.rating.toFixed(1)} ⭐</span>
              </div>
            </div>
          </div>
        )}
        ...
        {p.reason && (
          <div style={{ fontSize:13, color:'#374151', lineHeight:1.6, borderTop:'1px solid #F3F4F6', paddingTop:10 }}>
            {p.reason}
          </div>
        )}
      </div>
      ...
      <a href={buyUrl} target="_blank" rel="noopener noreferrer" style={{
        ...
        transition:'background .15s', borderTop:'1px solid #1D4ED8',
      }}
...
function SerpCard({ p }: { p:SerpProduct }) {
  // Use the direct product link from SearchAPI — this goes to the actual product page
  return (
    <a href={p.link} target="_blank" rel="noopener noreferrer" style={{
      ...
      <div style={{ padding:'10px 14px', background:'#F9FAFB', borderTop:'1px solid #F3F4F6', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <span style={{ fontSize:12, color:'#6B7280', fontWeight:500 }}>Direct link to {p.source || 'store'}</span>
        <span style={{ fontSize:13, color:'#2563EB', fontWeight:700, display:'flex', alignItems:'center', gap:4 }}>
          Buy now ↗
        </span>
      </div>
    </a>
  ...
  const startRecording = async () => {
    ...
    rec.onstop = async () => {
      ...
      try {
        ...
        const data = await res.json()
        if (data.transcript) { setInput(data.transcript); setRecState('idle'); submit(data.transcript) }
        else { setRecState('error'); setVoiceError(data.error || 'Could not understand. Try again.') }
      } catch { setRecState('error'); setVoiceError('Network error. Try again.') }
    }
    rec.start(250); setRecState('recording')
  }

  const toggleMic = () => recState === 'recording' ? stopRecording() : startRecording()
    ...
    <div style={{ maxWidth:960, margin:'0 auto', padding:'68px clamp(12px,4vw,20px) 60px' }}>
      ...
      <div style={{ marginBottom:28 }}>
        <div style={{
          ...
          />
          <button onClick={toggleMic} disabled={isProcessing} title="Voice search — 22 Indian languages"
            style={{ width:40, height:40, borderRadius:8, border:'none', flexShrink:0, background: isRecording ? '#EF4444' : '#F3F4F6', cursor: isProcessing ? 'not-allowed' : 'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s', animation: isRecording ? 'mic-pulse 1s ease infinite' : 'none' }}>
            {isProcessing
        ...
        {isRecording && <div style={{ marginTop:8, display:'flex', alignItems:'center', gap:6, fontSize:13, color:'#EF4444', fontWeight:500 }}><span style={{ width:7, height:7, borderRadius:'50%', background:'#EF4444', display:'inline-block', animation:'blink 1s infinite' }} />Recording... tap stop when done</div>}
      ...
      {!loading && answer && (
        <div>
          ...
          {/* AI Cards — now with Buy buttons */}
          {aiProducts.length > 0 && (
            <div style={{ marginBottom:36 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16, flexWrap:'wrap' }}>
                <span style={{ fontSize:16, fontWeight:700, color:'#111827' }}>Top Recommendations</span>
                <span style={{ fontSize:12, color:'#6B7280', background:'#F3F4F6', borderRadius:100, padding:'2px 10px', border:'1px solid #E5E7EB' }}>{aiProducts.length} products · AI-adjusted scores · Direct buy links</span>
              </div>
          ...
          {/* Live prices — direct links from SearchAPI */}
          {serpProducts.length > 0 && (
            <div style={{ marginBottom:36 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16, flexWrap:'wrap' }}>
                <span style={{ fontSize:16, fontWeight:700, color:'#111827' }}>Live Prices</span>
                <span style={{ fontSize:12, color:'#6B7280', background:'#F3F4F6', borderRadius:100, padding:'2px 10px', border:'1px solid #E5E7EB' }}>Direct links · Click to buy</span>
              </div>
