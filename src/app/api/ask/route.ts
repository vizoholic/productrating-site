...
export async function POST(req: NextRequest) {
  ...
  if (ct.includes('multipart/form-data')) {
    ...
    try {
      ...
      if (!sr.ok) return NextResponse.json({ error:`STT ${sr.status}: ${rt.slice(0,200)}`, transcript:'' }, { status:500 })
      let d: { transcript?:string } = {}
      try { d = JSON.parse(rt) } catch {}
      return NextResponse.json({ transcript: d.transcript || '' })
  ...
  if (!lang && hinglishRe.test(question)) lang = 'IMPORTANT: Respond in natural Hinglish — mix Hindi and English.'
...
- "rating": Your AI-adjusted score OUT OF 5. Must be between 1.0 and 5.0. Example: 4.2
- "platform_rating": The raw score shown on Amazon/Flipkart BEFORE fake review removal. Must be between 1.0 and 5.0. Should be slightly higher than "rating". Example: 4.6
- NEVER output any number above 5.0 for either rating field.
- "reviews": Total review count as string. Example: "2,341" or "1.8k"
- "price": In Indian Rupees. Example: "₹18,999"

INSTRUCTIONS:
1. Write 3-5 sentences of advice in plain text. No markdown, no asterisks, no bullets.
2. New line: ---PRODUCTS---
3. JSON array of 4-6 products:
[{"name":"Product Name","price":"₹XX,XXX","seller":"Amazon","rating":4.2,"platform_rating":4.6,"reviews":"2.3k","badge":"Best Pick","reason":"Why ideal for Indian buyers"}]
5. NEVER use <think> tags or **markdown**.`

  try {
    const res = await fetch('https://api.sarvam.ai/v1/chat/completions', {
      method:'POST',
      headers:{'Content-Type':'application/json','api-subscription-key':apiKey},
      body:JSON.stringify({ model:'sarvam-m', messages:[{role:'system',content:systemPrompt},{role:'user',content:`Question: ${question}\n${serpContext?`\nLive data:\n${serpContext}`:'\nUse your Indian market knowledge.'}`}], max_tokens:1400, temperature:0.4 }),
    })
    ...
    type AiP = {name:string;price:string;seller:string;rating:number;platform_rating:number;reviews:string;badge:string;reason:string}
    let aiProducts: AiP[] = []
    if (parts[1]) {
      try {
        ...
        if (m) {
          aiProducts = JSON.parse(m[0]).map((p: Record<string,unknown>) => ({
            ...
            platform_rating: Math.min(5.0, Math.max(1.0, Number(p.platform_rating) || Math.min(5.0, Number(p.rating)+0.3) || 4.3)),
          }))
        }
      } catch {}
    }
